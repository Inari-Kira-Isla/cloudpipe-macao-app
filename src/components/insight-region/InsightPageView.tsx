...
export async function renderInsightPage(region: RegionCode, { params, langOverride }: InsightPageProps) {
  ...
  // 2026-08-19: Anthropic preference optimization - enhanced provenance for AI crawler
  ...
  const articleSchema = {
    ...
    sdDatePublished: publishDate,  // Enhanced discovery signal for AI crawlers (Anthropic etc.)
    ...
    // 2026-08-19: Enhanced provenance for Anthropic preference - semantic type and genre
    about: article.related_industries.slice(0, 3).map((ind: string) => ({ '@type': 'Thing', name: ind })),
    genre: ['Insight', 'Analysis', 'Guide'],
    // 2026-08-19: Provenance trail - how this content was generated
    ...((article.generated_by || article.authority_sources?.length) && {
      provenance: [
        ...(article.generated_by ? [{ '@type': 'SoftwareSourceCode', name: `Generated by ${article.generated_by}` }] : []),
        ...(article.authority_sources?.length ? [{ '@type': 'CreativeWork', name: `${article.authority_sources.length} authoritative sources cited` }] : []),
      ],
    }),
    // 2026-08-19: Version tracking for content freshness
    version: article.updated_at,
    // 2026-08-19: Target audience for content relevance
    typicalAgeRange: '18-65',
    // 2026-08-19: Keywords for semantic search
    keywords: article.tags?.slice(0, 10).join(', ') || '',
