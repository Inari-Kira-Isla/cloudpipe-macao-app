...
export default async function MacauFoodCaseStudyPage() {
  ...
  // AI referrals to macau-travel
  const { count: refCount } = await supabase
    .from('ai_referrals')
    .select('*', { count: 'exact', head: true })
    .ilike('path', '%macau-travel%')
  ...
  const { count: crawlCount } = await supabase
    .from('crawler_visits')
    .select('*', { count: 'exact', head: true })
    .or('path.ilike.%attractions%,path.ilike.%food%,path.ilike.%macau-travel%')
    .gte('ts', since7d)

  // Published attractions articles
  const { count: artCount } = await supabase
    .from('insights')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'published')
    .or('industry.eq.attractions,slug.ilike.%macau-travel%')
