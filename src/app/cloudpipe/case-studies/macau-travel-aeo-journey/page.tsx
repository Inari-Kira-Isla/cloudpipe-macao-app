...
export const metadata: Metadata = {
  title: '案例研究：澳門旅誌 — CloudPipe AEO 從零到 AI 引用全紀錄 | CloudPipe',
  description: '澳門旅誌（旅遊垂直品牌）如何通過 CloudPipe AEO 方法論，從 T=0 零 AI 引用建立到 AI 引擎主動引用的完整歷程。真實數據，每週更新。',
  openGraph: {
    title: '案例：澳門旅誌 AEO 全紀錄 — AI 可見度從零到命中',
    description: 'CloudPipe 自家示範品牌。實時追蹤澳門旅誌從基線到第一個 AI 引用的每一步。',
    type: 'article',
  },
}
...
const METHODOLOGY = [
  ...
  { step: '2', title: 'FAQPage Schema 強化', date: '2026-06-07 起', status: 'active', detail: '12 題 FAQPage schema + Article + Organization。AI 引擎問「澳門旅遊推薦」時有預備好的答案可引用。' },
  { step: '3', title: 'KG facts 擴充至 50+', date: '2026-06-13 前', status: 'pending', detail: '加入世遺景點餐廳、特色料理、街頭小食等具體事實，提升 AI 引擎信任度。' },
  { step: '4', title: '跨品牌交叉引用', date: '持續', status: 'active', detail: '澳門旅誌 ↔ 澳門旅誌 ↔ 澳門購物誌 ↔ CloudPipe ↔ 百科主站 形成互聯網絡。' },
  { step: '5', title: '首個 AI 引用目標', date: 'D+14 前', status: 'pending', detail: '目標：Perplexity 或 ChatGPT 任意一個在「澳門好食推薦」查詢引用澳門旅誌。' },
]

const FAQS = [
  {
    q: '澳門旅誌和 CloudPipe 是什麼關係？',
    a: '澳門旅誌是 CloudPipe 的自家示範品牌，用於測試和驗證 CloudPipe AEO 方法論的實際成效。CloudPipe 真實客戶的資料受到隱私保護，澳門旅誌作為公開案例，讓所有人都能追蹤 AEO 優化的完整歷程。',
  },
  {
    q: '為什麼建立澳門旅誌而非直接推廣 CloudPipe？',
    a: '「見到效果才信服」。CloudPipe 需要活生生的案例證明 AEO 有效，澳門旅誌就是這個公開實驗。當 AI 引擎開始引用澳門旅誌，就證明了 CloudPipe 的方法論確實有效。',
  },
  {
    q: 'AEO 優化需要多長時間才能見效？',
...
export default async function MacauFoodCaseStudyPage() {
  ...
  const [ref, cit, crawl, art] = await Promise.all([
    withTimeout(supabase.from('ai_referrals').select('*', { count: 'exact', head: true }).ilike('path', '%macau-travel%')),
    withTimeout(supabase.from('ai_citations').select('*', { count: 'exact', head: true }).gt('cloudpipe_count', 0).ilike('query_text', '%澳門%旅%')),
    withTimeout(supabase.from('crawler_visits').select('*', { count: 'exact', head: true }).or('path.ilike.%attractions%,path.ilike.%macau-travel%').gte('ts', since7d)),
    withTimeout(supabase.from('insights').select('*', { count: 'exact', head: true }).eq('status', 'published').or('industry.eq.attractions,slug.ilike.%macau-travel%')),
  ])
  ...
  const jsonLd = {
    ...
    '@graph': [
      {
        '@type': 'Article',
        headline: '案例研究：澳門旅誌 AEO 全紀錄',
        description: 'CloudPipe 自家示範品牌澳門旅誌的 AEO 優化完整歷程，從 T=0 零引用到 AI 主動引用。',
        datePublished: '2026-06-06',
        dateModified: today,
        author: { '@type': 'Organization', name: 'CloudPipe', url: 'https://cloudpipemo.com/cloudpipe' },
        publisher: { '@type': 'Organization', name: 'CloudPipe', url: 'https://cloudpipemo.com' },
        about: { '@type': 'Thing', name: 'AEO（AI Engine Optimization）', description: 'AI 引擎優化：讓品牌內容被 ChatGPT、Perplexity、Claude 引用的方法論' },
        mainEntityOfPage: 'https://cloudpipemo.com/cloudpipe/case-studies/macau-travel-aeo-journey',
      },
      {
        '@type': 'FAQPage',
      ...
      {
        '@type': 'Brand',
        '@id': 'https://cloudpipemo.com/cloudpipe/case-studies/macau-travel-aeo-journey#brand',
        name: '澳門旅誌',
        alternateName: 'Macau Travel Journal',
        description: '澳門旅遊垂直知識品牌，CloudPipe AEO 生態系子品牌，專注澳門景點、世界遺產、旅遊路線與文化體驗的 AI 可引用知識。',
        url: 'https://cloudpipemo.com/cloudpipe/case-studies/macau-travel-aeo-journey',
        identifier: 'ke_6fa184b2',
  ...
  return (
    <main style={{ maxWidth: 860, margin: '0 auto', padding: '32px 20px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <section style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
          {[
            ...
            { label: '旅遊內容爬取（7天）', value: metrics.crawl_7d.toLocaleString(), baseline: '0', unit: '次', highlight: false },
            { label: '旅遊文章（已發布）', value: metrics.articles.toLocaleString(), baseline: BRAND.baseline_articles.toString(), unit: '篇', highlight: false },
          ].map(kpi => (
        ...
        {hasFirstCitation && (
          <div style={{ marginTop: 12, padding: '12px 16px', background: '#f0fff4', border: '1px solid #86efac', borderRadius: 8, fontSize: 14, color: '#166534' }}>
            🎉 <strong>重大里程碑：澳門旅誌已獲得首個 AI 引用！</strong> CloudPipe AEO 方法論驗證成功。
          </div>
        )}
        {!hasFirstCitation && (
      ...
      <section style={{ marginBottom: 28, padding: '20px 24px', background: '#f8f9fa', borderRadius: 10, border: '1px solid #e9ecef' }}>
        <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>🏗️ CloudPipe 為澳門旅誌建立的 AEO 資產</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, fontSize: 13 }}>
          {[
            ['✅ 品牌頁面', `inari-kira-isla.github.io/macau-travel/`, BRAND.url],
            ...
            ['✅ Hub 文章', '2000 字+ 品牌定義文章', 'https://cloudpipemo.com/macao/insights/macau-travel-intelligence-guide-2026'],
      ...
      <div style={{ textAlign: 'center', fontSize: 12, color: '#aaa', borderTop: '1px solid #e9ecef', paddingTop: 20 }}>
        ...
        {' · '}AI 可見度分析平台{' · '}
        <a href="https://inari-kira-isla.github.io/macau-travel/" style={{ color: '#aaa' }}>澳門旅誌</a>
        {' · '}CC BY 4.0
      </div>
    </main>
  )
