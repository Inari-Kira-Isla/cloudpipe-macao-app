...
// TW 現時 0 篇）。TW 冇 ms 內容唔構成風險：getInsight 搵唔到 row 就 notFound()，
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildMetadata('TW', { params: Promise.resolve({ slug }), langOverride: lang })
}

export default async function TaiwanInsightLangPage({ params }: PageProps) {
  const { lang, slug } = await params
  if (!VALID_LANG_PATHS.includes(lang as LangPath)) notFound()
  return renderInsightPage('TW', { params: Promise.resolve({ slug }), langOverride: lang })
}
