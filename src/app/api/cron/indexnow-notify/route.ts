...
async function getRecentUrls(): Promise<string[]> {
  const baseUrl = 'https://cloudpipemo.com';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const keyLocation = `https://cloudpipemo.com/indexnow-key-${indexNowKey}.txt`;
    ...
    const payload: IndexNowPayload = {
      host: (process.env.NEXT_PUBLIC_SITE_URL || 'https://cloudpipemo.com').trim().replace(/^https?:\/\//, ''),
      key: indexNowKey,
