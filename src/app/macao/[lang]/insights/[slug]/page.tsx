...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildMetadata('MO', { params: Promise.resolve({ slug }), langOverride: lang })
}

export default async function MacaoInsightLangPage({ params }: PageProps) {
  const { lang, slug } = await params
  if (!VALID_LANG_PATHS.includes(lang as LangPath)) notFound()
  return renderInsightPage('MO', { params: Promise.resolve({ slug }), langOverride: lang })
}
