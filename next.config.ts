...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    ...
    // ── Region insight redirects: HK/TW/JP/GLOBAL articles served under /macao/ until region routes are built ──
    // These URLs are generated by macao_insight_generator.py and visited by AI crawlers.
    // NOTE: Do NOT add reverse redirects (/macao/insights/hk-* → /hongkong/insights/*) until
    // the /hongkong/, /taiwan/, /japan/ Next.js routes actually exist — would cause infinite loops.
    for (const region of ['hongkong', 'taiwan', 'japan', 'global']) {
      redirects.push({ source: `/${region}/insights/:slug*`, destination: `/macao/insights/:slug*`, permanent: false })
      redirects.push({ source: `/${region}/:path*`, destination: `/macao/:path*`, permanent: false })
    }
